<!DOCTYPE HTML>
<html>

<head>
        <title>Restaurant Ratatouille</title>
        <meta charset="utf-8">
        <meta name="Restaurant Ratatouille cuisine traditionnelle Liscence ratatouille" content="165c. uniques">
        
        <link href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.1/css/bootstrap.min.css" rel="stylesheet">
        <link href="fds.css" rel="stylesheet">

</head>

<body class="index">
    <div id="page" class="container-fluid" >
    
<?php include ("header.php") ?>
          
  
    <div class="row">
        <h1 class="col-xs-12">Restaurant Ratatouille</h1>
        <h2 class="col-xs-offset-1 col-xs-10">Le goût de la tradition</h2>
     
    </div>

<div class="panel panel-default col-xs-12 col-md-7">
    <div class="panel-heading">L'actualité du jour</div>
    <div class="panel-body">
        <h3>Ouverture de notre restaurant à Fontainebleau</h3>
        <div class="row">
        <p class="col-xs-4"><img src="img/ouverture_restaurant.jpg"/></p>
        <p class="col-xs-8">Notre nouveau restaurant ouvrira très prochainement à Fontainelbeau!<br/> Dès le 1er octobre, venez découvrir notre chef et sa carte, comme toujours dans le respect de la qualité.</p>
        </div>

    
    </div>
  </div>
    
 
</div>
       
        
    
</body>
<?php include ("footer.php") ?>
</html>